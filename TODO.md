...
- [x] (Optional) Point a custom domain (`mice.ordulis.com`) at the Cloudflare Workers deployment via the dashboard — PartyKit's `.partykit.dev` subdomain stays as-is (managed PartyKit doesn't support custom domains; cross-origin client↔server is expected and correct per the GDD)
